    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header py-3">
            <h4 class="modal-title">Retailer List</h4><span id="mailresponce"></span>
            <button type="button" class="close m-0 p-0" data-dismiss="modal">&times;</button>         
        </div>
        <div class="modal-body py-2 storemodalbody"> 
...
<?php
    ...
    array_push($headingsTable, array( 'data' => 'game_id', 'className' => 'text-right' ) );
...
@push('before-scripts') 
    ...
    <script type="text/javascript"> 
                    ...
                    { targets : <?php echo $target_count; ?>,  
                        "render": function(data, type, row) { 
                           console.log('Data  = '+  data      );
                            var url = '/view';
                            return ' <button onclick="getStore('+ data +');" type="button" class="cbtn csuccess btn-icon" data-id="'+ data +'" data-toggle="modal" data-target="#storemonitor_modal" ><i class="mdi mdi-information-variant"></i></button>';
                        }, 
                    }
                 ],
                 rowReorder: {
		...
		function sendEmailRequestToaddGame(storeEmail,gameid){
			$("#mailresponce").html("Please wait for email...!");
			 $.ajax({
				type:'POST',
				url:"{{URL::to('/')}}/admin/storemonitor/sendemailrequesttoaddgame",
				data: {
					"_token": "{{ csrf_token() }}",
				...
				success: function(response)
				{
					if(response.status == 'ok'){
						$("#mailresponce").addClass("text-success");
						$("#mailresponce").removeClass("text-danger");
						$("#mailresponce").html("Your request sent to store manager successfully!");
					}else{
						$("#mailresponce").addClass("text-danger");
						$("#mailresponce").removeClass("text-success");
						$("#mailresponce").html("!Ooops something went wring, please try again");
					}
					setTimeout(function(){ $("#mailresponce").html(''); }, 3000);
				}
			});
			return false;
		}
		function getStore(gameId){
			 $.ajax({
				type:'GET',
				url:"{{URL::to('/')}}/admin/storemonitor/storelist/"+gameId,
				//url: '<?php echo URL::to('storemonitor.storelist'); ?>/'+gameId,
				dataType: "html",
				async: false,
				cache: false,
				success: function(response)
				{
					$(".storemodalbody").html(response);		
				}
			});
			return true;
		}
